 ...
 * JCanvas
 * 给canvas里面图形加上常用事件代理
 * @author <hongru.chenhr@gmail.com>
 */ 
;(function () {
    // mobile navigator
    ...
    var ObjectContainerClass = InteractiveClass.extend({
        ...
        dispatchMouseEvent: function (type, x, y) {
            ...
            for (var i = this.children.length-1; i >= 0; i--) {
                ...
                //鼠标悬浮于子对象上面
                if (isMouseover(child) && child.isVisible()) {
    ...
    var Stage = ObjectContainerClass.extend({
        init: function (canvas, option) {
            ...
            if (canvas === undefined) {
                throw new Error('htmlCanvasElement undefined')
            }
            this.canvas = canvas;
            this.isStart = false;
            this.interval = 16;
    ...
    var Sprite = ObjectContainerClass.extend({
        ...
        render: function (rd) {
            ...
            // 强制缩放，保证子对象不会比自己大
            this.ctx.scale(
            this.width < this.maxWidth ? this.width / this.maxWidth : 1, this.height < this.maxHeight ? this.height / this.maxHeight : 1);
            // 绘制子对象
            for (var i = 0; i < this.children.length; i++) {
                this.ctx.translate(this.children[i].x, this.children[i].y);
                this.children[i].render(rd);
                this.children[i].ctx.translate(-this.children[i].x, -this.children[i].y);
            }
            this.ctx.scale(
            this.width < this.maxWidth ? this.maxWidth / this.width : 1, this.height < this.maxHeight ? this.maxHeight / this.height : 1);
        },
        onDrag: function (x, y) {
            var context = this;
    ...
    var Color = Class.extend({
        ...
        add: function (c) {
            return new Color(
            Math.min(this.r + c.r, 1), Math.min(this.g + c.g, 1), Math.min(this.b + c.b, 1));
        },
        subtract: function (c) {
            return new Color(
            Math.max(this.r - c.r, 0), Math.max(this.g - c.g, 0), Math.max(this.b - c.b, 0));
        },
        multiply: function (n) {
            return new Color(
            Math.min(this.r * n, 1), Math.min(this.g * n, 1), Math.min(this.b * n, 1));
        },
        divide: function (n) {
            ...
            var pointA = a,
                ...
                triangle = CVS.createSprite(ctx, function () {
                    ...
                    this.isPointInside = function (x, y) {
                        ...
                        if (
                        fAB(pointA, pointB, pointC) * fBC(pointA, pointB, pointC) > 0 && fBC(pointA, pointB, pointC) * fCA(pointA, pointB, pointC) > 0) return true;
