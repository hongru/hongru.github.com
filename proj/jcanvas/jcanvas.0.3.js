		  ...
		  var pointA = a,
			  ...
			  triangle = CVS.createSprite(ctx, function () {
				...
				this.isPointInside = function (x, y) {
					var fAB = function (p1, p2, p3) { return (y - p1.screenY) * (p2.screenX - p1.screenX) - (x - p1.screenX) * (p2.screenY - p1.screenY); }; 
					var fCA = function (p1, p2, p3) { return (y - p3.screenY) * (p1.screenX - p3.screenX) - (x - p3.screenX) * (p1.screenY - p3.screenY); }; 
					var fBC = function (p1, p2, p3) { return (y - p2.screenY) * (p3.screenX - p2.screenX) - (x - p2.screenX) * (p3.screenY - p2.screenY); };
					
					if ( 
						fAB(pointA, pointB, pointC) * fBC(pointA, pointB, pointC) > 0 && 
						fBC(pointA, pointB, pointC) * fCA(pointA, pointB, pointC) > 0 
					) return true; 
